...
void function GamemodeJugg_Init()
{
	...
	SetRoundWinningKillReplayKillClasses( true, false ) // only track titan kills
...
void function MovePlayers(){
	array<entity> players = GetPlayerArray()

	foreach( entity player in players ){
		SetTeam( player, TEAM_MILITIA )
		player.RespawnPlayer( null )
		RespawnAsPilot( player , false )
	}

	SetTeam( players[ RandomInt( players.len() ) ], TEAM_IMC )

	RespawnAsTitan( GetPlayerArrayOfTeam( TEAM_IMC )[0] , false )
